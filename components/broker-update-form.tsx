...
import { updateBrokerAction } from "@/app/_actions/_brokerActions";
...
const BrokerUpdateForm = ({ broker }: { broker: Broker }) => {
  ...
  const form = useForm<z.infer<typeof BrokerSchema>>({
    ...
    defaultValues: {
      ...
      email: broker.email || "",
      phone: broker.phone || "",
      percentage: broker.percentage || 0,
    },
  });
  function onSubmit(data: z.infer<typeof BrokerSchema>) {
    startTransition(() => {
      updateBrokerAction(data, broker.id).then((data) => {
        const res = JSON.parse(data);
        if (res.updatedBroker) {
          toast({
            title: "Success",
            description: <ToastDesc>Broker Updated Successfully</ToastDesc>,
          });
          return router.back();
        } else {
          toast({
...
export default BrokerUpdateForm;
